...
public class 실습_5_3_배열필드 {
	class Person {
	    ...
	    void show() {
	    	...
	    	System.out.print("기본 정보 출력 : name = " + name + "age = " + age + "weight = " + weight);
	        // 교육과목 및 수강 연도 출력
	    	for(int i = 0; i < subjects.length; i++) {
	    		System.out.println("교육과목 : " + subjects[i] + "수강 연도 : " + years[i]);
	    	}
	    	// while 문을 사용하여 교육과목과 수강연도를 출력
	    	while(count == subjects.length) {
	    		System.out.println("교육과목 : " + subjects[count] + "수강 연도 : " + years[count]);
	    		count++;
	    	}
	    }
